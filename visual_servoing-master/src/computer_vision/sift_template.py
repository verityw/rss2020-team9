...
def cd_sift_ransac(img, template):
	...
	if len(good) > MIN_MATCH:
		...
		########## YOUR CODE STARTS HERE ##########
                ...
                transformed_ys = []
                for pt in pts:
                        pt_3d = np.append(pt, 1) # z = 1
                        transformed_pt = np.dot(M, pt_3d)
                        transformed_xs.append(transformed_pt[0])
                        transformed_ys.append(transformed_pt[1])

                # transformed pts might not be in a rectangle anymore
                x_min = min(transformed_xs)
                y_min = min(transformed_ys)
                x_max = max(transformed_xs)
                y_max = max(transformed_ys)
...
def cd_template_matching(img, template):
	...
	for scale in np.linspace(1.5, .5, 50):
		...
		# x1,y1 pixel will be accurate, but x2,y2 needs to be correctly scaled
                corr = cv2.matchTemplate(img_canny, resized_template, cv2.TM_CCOEFF_NORMED)
                (_, maxVal, _, maxLoc) = cv2.minMaxLoc(corr)

                if best_match is None or maxVal > best_match:
                    best_match = maxVal
                    bounding_box = ((maxLoc[0], maxLoc[1]),
